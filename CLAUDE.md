...
- **A small working context is a goal, not a limitation.** `_MAX_TOKENS` (32k, `app/assistant/history.py`) is a deliberate budget, far below the model's real window — for cost, and because a long context makes the agent follow its instructions worse. So `[Context: N% used]` measures against that budget: over 100% means the agent has outgrown its intended footprint, which is true and is meant to push it to prune. **This is why the agent has memory tiers at all** — long-term memories, lists, saved prompts, and transcript pruning exist so it can fetch what a moment needs instead of carrying everything forever. Widening the budget instead of retrieving is the wrong repair every time.
...
- **A dependency is bound in a constructor, never passed per call.** A function that takes a client,
  ...
  primitives stay free functions. Enforced, not just documented: **ANON003** (`baski_lint`, run by
  `make lint`) fails on a module-level function that takes a client/database/bot/store; the type
  names it counts are declared under `[tool.anon_lint]` here. A genuine exception needs
  `# noqa: ANON003` naming why.
