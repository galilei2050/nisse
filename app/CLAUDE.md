...
app/
  ...
  assistant/        the main agent — composition root
    ...
    history.py      MongoMessageHistory — transcript in Mongo `conversation_turns`, one doc per turn (soft-deleted when pruned); turns are `MongoTurn` (baski `Turn` + `created_at`); format_for_api strips thinking blocks from settled turns (Opus 4.5+ bills replayed thinking), marks the prompt-cache breakpoint on the last turn (baski `mark_cached`), and stamps each `[Turn N]` marker with the turn's absolute UTC send-time on the first turn and after a >1h gap (`_turn_marker`) so the model can judge recency — absolute (never relative) to stay byte-stable in the cached prefix, normalized via baski `as_utc`; the volatile `[Context: N% used]` footer rides after the breakpoint via `context_status()`; `truncate()` only RECORDS each call's size (via baski `effective_input_tokens`, incl. the cached prefix — raw `input_tokens` is shrunk by caching), while **`compact()`** is the one method that shrinks the transcript — called from `Conversation.reply` once the agent loop has finished (in a `finally`, so a failed run still pays its bookkeeping), never inside the loop, which must only ever see the transcript grow. Three cuts: a turn with no text at all always goes (nothing was said in it) → **over budget**, turns past `_PAYLOAD_RETENTION` lose their tool calls, results, attachments and thinking but keep their text (`MongoTurn.keep_only_text`; those payloads were 45% of the live context when measured) → over budget with nothing left to strip, whole turns go, oldest first. Both costly cuts wait for a real measurement, so a small conversation keeps every photo however old — and `load()` restores turns whole for the same reason. After any cut `_last_input_tokens` is zeroed: it described the pre-cut transcript, and left standing it makes `context_status()` report a fullness that no longer exists, which reads to the agent as an order to prune. A tool call and its result share one turn, so reducing one never orphans a `tool_use`. Compaction is in-memory ONLY — the durable write is handed a snapshot of the turn taken when it completed (`__exit__`), so a turn stripped before its own fire-and-forget write lands still reaches Mongo whole; `deleted_at` and `message_ids` remain the only fields that ever change on a stored turn. `_forget()` is the only way a turn leaves the transcript — compaction and `delete_turns` (the agent's `prune_transcript`) both go through it, with a `ForgetReason` on the log line. `add_photo`/`add_document` append a user image/PDF message the model reads natively; its base64 block is persisted like any other, so the image survives a reload and stays in context for follow-ups — until an over-budget compaction strips it along with the other payloads. `link_messages` stamps the Telegram message ids the answer was delivered in onto the newest turn (called by the chat router AFTER `flush()` — it deliberately does not upsert, so it can't create a turn doc missing its audit fields); `TurnLookup` is the reverse read, message → turn, used by the reaction recorder
