...
app/
  ...
  assistant/        the main agent — composition root
    ...
    history.py      MongoMessageHistory — transcript in Mongo `conversation_turns`, one doc per turn (soft-deleted when pruned); turns are `MongoTurn` (baski `Turn` + `created_at`); format_for_api strips thinking blocks from settled turns (Opus 4.5+ bills replayed thinking), marks the prompt-cache breakpoint on the last turn (baski `mark_cached`), and stamps each `[Turn N]` marker with the turn's absolute UTC send-time on the first turn and after a >1h gap (`_turn_marker`) so the model can judge recency — absolute (never relative) to stay byte-stable in the cached prefix, normalized via baski `as_utc`; the volatile `[Context: N% used]` footer rides after the breakpoint via `context_status()`; `truncate()` only RECORDS each call's size (via baski `effective_input_tokens`, incl. the cached prefix — raw `input_tokens` is shrunk by caching), while **`compact()`** is the one method that shrinks the transcript — called once per reply from `Conversation.reply`, AFTER the answer is delivered, so a reply keeps every turn it started with and the loop always sees an append-only transcript (shrinking mid-run both cuts context out from under a reply mid-thought and re-writes the whole cache at 1.25x on every remaining turn). `compact()` makes three cuts, each only if the cheaper one wasn't enough: a pure tool round always goes (it said nothing) → a turn past `_PAYLOAD_RETENTION` gives up its tool data but keeps its words (`MongoTurn.keep_only_text`; those payloads are ~3/4 of the bulk and were consumed by the answer that used them) → a context still near the budget gives up whole exchanges. A tool call and its result share one turn, so reducing one never orphans a `tool_use`. Turns leave the transcript through exactly one door, `_forget()`, which is also what `delete_turns` (the agent's `prune_transcript`) goes through. `add_photo`/`add_document` append a user image/PDF message the model reads natively; its base64 block is persisted like any other, so the image survives a reload and stays in context for follow-ups — until it is shed with the rest of the machinery. `link_messages` stamps the Telegram message ids the answer was delivered in onto the newest turn (called by the chat router AFTER `flush()` — it deliberately does not upsert, so it can't create a turn doc missing its audit fields); `TurnLookup` is the reverse read, message → turn, used by the reaction recorder
