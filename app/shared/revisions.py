...
agent that made it. So every write appends a `Revision` here — a content-losing one carrying the
text about to disappear, a create carrying only what it added.
...
class Attribution:
    """Who is writing right now, and under which run (none when the live assistant writes)."""
...
def acting_as(actor: Actor, *, run_id: str) -> Generator[None]:
    ...
    Wrap anything that writes and is not the live agent — a curator pass, an admin script — or the
    history will name the assistant for edits it never made.
    """
...
class RevisionLog:
    ...
    async def ensure_indexes(database: AsyncDatabase) -> None:
        """By run — how a finished pass counts what it changed, and how the owner reads a night."""
