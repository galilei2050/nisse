...
from app.curator.classify import MessageClassifier, MessageKind
...
def test_a_fenced_json_block_parses() -> None:
    ...
    parsed = MessageClassifier._parse(raw)
...
def test_unusable_output_raises_instead_of_reading_as_a_quiet_night() -> None:
    with pytest.raises(ValueError, match="unusable output"):
        MessageClassifier._parse("I looked at the conversation and found three corrections.")
